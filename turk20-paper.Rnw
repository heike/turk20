...
\item identify minor and major semi axes of ellipses
\item include discussion of color scheme: * essentially all the same; * but original study was a bit better based on Heer's color scheme + tweaks
\end{enumerate}
}
...
<<getellipses, echo=FALSE>>=
dframe <- subset(pics, data_name.y=="set-16-k-5-sd.trend-0.45-sd.group-0.20-turk16")

dy <- read.csv(file.path("data/lineup-data/turk19", paste0(dframe$data_name.y[1],".csv")))

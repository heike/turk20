...
d19 <- file.path("data/lineup-data/turk19", dir("data/lineup-data/turk19"))
d16 <- file.path("data/lineup-data/turk16", dir("data/lineup-data/turk16"))
...
pics <- merge(pics16, pics19, by=c("test_param", "k", "set", "sdtrend", "sdgroup"))
...
list_of_pics16 <- lapply(1:nrow(pics), function(i) {
  ...
  dx <- read.csv(file.path("data/data16", paste0(fname,".csv")))
  k <- length(unique(dx$group))
  cols <- colors[ks[[k]]]
  xmax =max(dx$x)
  ymin = min(dx$y)
  qplot(data=dx, x, y, colour = factor(group), size=I(3), facets=~.sample) +
    scale_colour_manual(values = cols) + theme_lineup() +
    geom_text(data=subset(res16, pic_id == pics$pic_id.x[i]), 
...
list_of_pics19 <- lapply(1:nrow(pics), function(i) {
  ...
  dx <- read.csv(file.path("data/data19", paste0(fname,".csv")))
  k <- length(unique(dx$group))
  cols <- colors[ks[[k]]]
  xmax =max(dx$x)
  ymin = min(dx$y)
  qplot(data=dx, x, y, colour = factor(group), size=I(3), facets=~.sample) +
    scale_colour_manual(values = cols) + theme_lineup() +
    geom_text(data=subset(res19, pic_id == pics$pic_id.y[i]), 
              aes(label = round(counts[[1]],2)), x = xmax, y = ymin, size = 5, 
            colour="grey70", hjust="inward", vjust="inward")
})
